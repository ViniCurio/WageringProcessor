...
A API fica em `http://localhost:3000`. PostgreSQL usa `5432` e LocalStack usa `http://localhost:4566`. Os defaults estão em `.env.example`.
...
Use `bun run migration:down` para reverter a última migration. Decisões estão em `ARCHITECTURE.md` e o guia de código em `COMENTARIOS.md`.
